...
public interface VentaRepository extends JpaRepository<Venta, Long> {
    ...
    @Query("""
            select new pe.edu.upeu.PharmaBackckend.dto.reporte.VentaPorCategoriaDTO(
                       cat.id,
                       cat.nombre,
                       sum(d.cantidad),
                       sum(d.subtotal))
            ...
            where v.estado = pe.edu.upeu.PharmaBackckend.enums.EstadoVenta.REGISTRADA
              and (:desde is null or v.fecha >= :desde)
              and (:hasta is null or v.fecha <= :hasta)
    ...
    @Query("""
            select new pe.edu.upeu.PharmaBackckend.dto.reporte.ProductoMasVendidoDTO(
                       p.id,
                       p.nombre,
                       cat.nombre,
                       sum(d.cantidad),
